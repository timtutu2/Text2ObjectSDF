...
def main():
    ...
    for epoch in range(train_cfg['num_epochs']):
        ...
        for batch_idx, (points, sdf_gt, prompts) in enumerate(dataloader):
            ...
            # no_sync() suppresses DDP gradient all-reduce during accumulation steps,
            # allowing gradients to accumulate locally before the update step.
            sync_context = (
                sdf_decoder.no_sync()
            ...
            with sync_context:
                ...
                # CRITICAL: at update steps the DDP gradient all-reduce fires during
                # backward(). Every rank must call backward() or every rank must skip —
                # any mismatch causes an NCCL collective mismatch → 10-min watchdog
                # timeout → SIGABRT. Use an all-reduce MIN so that if ANY rank has a
                # non-finite loss, ALL ranks skip backward for this update step.
            ...
            if is_update_step:
                ...
                if is_main_process:
                    if total_val != total_val:  # NaN check
                        print(
